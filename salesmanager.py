...
def salesManager():
    ...
    menuWindow.title("Sales Manager")

    bigFrame=tk.Label(menuWindow, background=v.c2)
    bigFrame.grid()
    menuHeading=tk.Label(bigFrame, text="Sales Manage", font=v.fontSimple, background=v.c2)
    menuHeading.grid(row=0, column=0, sticky="W")
    subHeading=tk.Label(bigFrame, text="Show all the sales going on, till Date.", font=v.fontCaption, background=v.c2, foreground=v.c4)
