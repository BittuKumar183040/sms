...
def purchaseManage():
    ...
    menuWindow.title("Purchase Manager")

    bigFrame=tk.Label(menuWindow, background=v.c2)
    bigFrame.grid()
    menuHeading=tk.Label(bigFrame, text="Purchase Manage", font=v.fontSimple, background=v.c2)
    menuHeading.grid(row=0, column=0, sticky="W")
    subHeading=tk.Label(bigFrame, text="List all the purchased items", font=v.fontCaption, background=v.c2, foreground=v.c4)
